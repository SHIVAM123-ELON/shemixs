...
import { teacherProfile } from '@/lib/portal/teacher-data';
      ...
      <Tabs defaultValue="account">
          ...
          <SectionCard title="Profile Information" icon={<Icons.User className="h-4 w-4 text-primary" />}>
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              <div>
                ...
                <Input id="name" defaultValue={teacherProfile.name} className="mt-1.5" />
              </div>
              <div>
                <Label htmlFor="email">Email</Label>
                <Input id="email" type="email" defaultValue={teacherProfile.email} className="mt-1.5" />
              </div>
              <div>
                <Label htmlFor="phone">Phone</Label>
                <Input id="phone" defaultValue={teacherProfile.phone} className="mt-1.5" />
              </div>
              <div>
                <Label htmlFor="department">Department</Label>
                <Input id="department" defaultValue={teacherProfile.department} disabled className="mt-1.5" />
              </div>
            </div>
            <div className="mt-4">
        ...
        <TabsContent value="notifications">
          <SectionCard title="Notification Preferences" icon={<Icons.Bell className="h-4 w-4 text-primary" />}>
            <div className="space-y-4">
              {([
                { key: 'classes', label: 'Class Updates', desc: 'Notifications about class schedules and live sessions' },
                { key: 'assignments', label: 'Assignment Alerts', desc: 'New submissions, late submissions, and grading reminders' },
                { key: 'tests', label: 'Test Notifications', desc: 'Test creation, scheduling, and auto-grading completion' },
                { key: 'results', label: 'Result Alerts', desc: 'Pending result publication and student score updates' },
                { key: 'general', label: 'General Announcements', desc: 'School-wide announcements and administrative updates' },
              ] as const).map((item) => (
        ...
        <TabsContent value="privacy">
          <SectionCard title="Privacy Settings" icon={<Icons.Shield className="h-4 w-4 text-success" />}>
            <div className="space-y-4">
              {[
                { label: 'Show profile to students', desc: 'Allow students to view your profile and contact info' },
                { label: 'Show availability to parents', desc: 'Let parents see your available meeting slots' },
                { label: 'Allow direct messages', desc: 'Let students and parents send you direct messages' },
                { label: 'Show teaching stats publicly', desc: 'Display your teaching statistics on your public profile' },
              ].map((item, i) => (
