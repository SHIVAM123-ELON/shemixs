...
import { teacherCalendarEvents } from '@/lib/portal/teacher-data';
...
export default function TeacherCalendarPage() {
  ...
  const weekEvents = teacherCalendarEvents.filter((e) => {
    const d = new Date(e.date);
    return d >= new Date('2025-07-28') && d <= new Date('2025-08-03');
  });

  return (
    <>
      <PageHeader title="Calendar" description="View your classes, assignments, tests, and meetings in one place." />
      ...
      <div className="mb-6 flex gap-2">
        <button
          ...
          className={cn(
            ...
            view === 'monthly' ? 'bg-primary text-primary-foreground' : 'border border-border text-muted-foreground hover:bg-muted'
          )}
        >
          Monthly
        </button>
        <button
          ...
          className={cn(
            'rounded-lg px-3 py-1.5 text-sm font-medium transition-colors',
            view === 'weekly' ? 'bg-primary text-primary-foreground' : 'border border-border text-muted-foreground hover:bg-muted'
          )}
        >
          Weekly
        </button>
                  ...
                  return (
                    <div key={day} className="min-h-[120px] rounded-lg border border-border p-2">
                      <div className="mb-2 text-center">
                        ...
                        <p className="text-sm font-bold text-foreground">{weekDates[i]}</p>
                      </div>
                      <div className="space-y-1">
                        {dayEvents.map((ev) => {
                          ...
                          return (
                            <div
                              key={ev.id}
                              className={cn('rounded border px-1.5 py-1 text-xs', eventTypeColor[ev.type])}
                            >
              ...
              .map((ev) => {
                ...
                return (
                  <div key={ev.id} className="flex items-center gap-3 rounded-lg border border-border p-3">
                    <div className={cn('flex h-9 w-9 shrink-0 items-center justify-center rounded-lg border', eventTypeColor[ev.type])}>
