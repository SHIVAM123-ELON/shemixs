...
import type { ParentCalendarEvent } from '@/lib/portal/parent-types';

interface ParentCalendarWidgetProps {
  events: ParentCalendarEvent[];
}

const eventTypeDot: Record<ParentCalendarEvent['type'], string> = {
  exam: 'bg-destructive',
  class: 'bg-primary',
  fee: 'bg-warning',
  meeting: 'bg-accent',
...
export function ParentCalendarWidget({ events }: ParentCalendarWidgetProps) {
  const [selected, setSelected] = useState<Date | undefined>(new Date('2025-08-02'));

  const eventDates = new Map<string, ParentCalendarEvent[]>();
  events.forEach((ev) => {
    const key = ev.date;
