...
import { teacherProfile, teacherNotifications } from '@/lib/portal/teacher-data';

export function TeacherTopNav({ onMenuClick }: { onMenuClick: () => void }) {
          ...
          <DropdownMenuContent align="end" className="w-56">
            <DropdownMenuLabel>
              <div className="flex flex-col">
                <span className="text-sm font-semibold">{teacherProfile.name}</span>
                <span className="text-xs text-muted-foreground">{teacherProfile.email}</span>
              </div>
            </DropdownMenuLabel>
            <DropdownMenuSeparator />
            <DropdownMenuItem asChild>
              <Link href="/portal/teacher/profile">Profile</Link>
            </DropdownMenuItem>
            <DropdownMenuItem asChild>
